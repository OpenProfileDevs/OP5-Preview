...
function loadUserDataPublic() {
    ...
    fetch(`/data/profile/${urlprofile}.json`)
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            ...
            <p id="profile_username" style="display: relative; font-size: 24px; font-family: NotoSans-Medium; margin-bottom: 60px; transform: translate(0px, -135px)">@${data.username}</p>
            <p style="display: relative; font-size: 24px; font-family: NotoSans-Medium;">Joined ${formattedJoinDate}</p>
            `;
...
function createEditProfileButton(data) {
    ...
    button.addEventListener('click', function() {
        ...
        if (newDataValue !== null && newDataValue.trim() !== '') {
            ...
            // Fetch the blacklist of words
            fetch('/blacklist.txt')
                .then(response => response.ok ? response.text() : Promise.reject('Failed to fetch blacklist'))
                .then(blacklist => {
...
// Function to check if newDataValue contains blacklisted words
