            ...
            .then(data => {
                if (data.loggedIn) {
                    ...
                    if (userData) {
                        ...
                        const sessionData = `➡️ **@${userData.username}** viewed **${specificElementData}**'s profile from **${name}** ${emoji}`;
                    ...
                    console.log('Specific Element Data:', specificElementData);
                    const sessionData = `➡️ **Guest** viewed **${specificElementData}**'s profile from **${name}** ${emoji}`;
                    // Send message to server with session data
                    sendSessionDataToServer(sessionData);
                }
            })
