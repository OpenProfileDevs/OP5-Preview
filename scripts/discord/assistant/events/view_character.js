const userSessions = {};
var urlEnding = window.location.pathname.split('/').pop();
var character_name = document.getElementById('full_name').value;
...
// Example usage
async function handleUserJoinChar() {
            ...
            .then(data => {
                if (data.loggedIn) {
                    ...
                    if (userData) {
                        ...
                        const sessionData = `➡️ **@${userData.username}** viewed **${urlEnding}** from **${name}** ${emoji}`;
                        // Send message to server with session data
                        sendSessionDataToServer(sessionData);
                    }
                } else {
                    // If not logged in, handle accordingly
                    const sessionData = `➡️ **Guest** viewed **${urlEnding}** from **${name}** ${emoji}`;
                    // Send message to server with session data
                    sendSessionDataToServer(sessionData);
                }
            })
