...
document.addEventListener('click', async function(event) {
    ...
    try {
        ...
        if (element.id && element.id.startsWith('restoreprofile_')) {
            const authorId = element.id.replace('restoreprofile_', ''); // Get author id from element id

            try {
                // Directly make a request to delete the profile
                const deleteResponse = await fetch(`/restore-profile/${authorId}/${token}`, {
                    method: 'DELETE'
                });
